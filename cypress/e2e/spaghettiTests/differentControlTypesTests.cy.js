...
describe("Tests for different control types", () => {
  it("Drop-down with select tag test", () => {
    ...
    cy.xpath("//div[@id='uniform-selectProductSort']/span").should(
      "have.text",
      "In stock"
    );
  });
  ...
  it("Handling Iframes", () => {
    ...
    cy.getIframe("#mce_0_ifr").clear().type("Hello word!!! {ctrl+a}");
    ...
    cy.get("iframe[name='result']", { timeout: 5000 })
      ...
      .then(($iframe) => {
        ...
        cy.wrap($body)
          ...
          .type("London");
        cy.wrap($body).find(".pac-container").should("be.visible");
        cy.wrap($body).find(".pac-item:first").click();
        cy.wrap($body).find(".pac-container").should("not.be.visible");
      });
  });
});
